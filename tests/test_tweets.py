...
def test_post_tweet_success(mock_create_tweet):
    mock_create_tweet.return_value = MagicMock(data={'id': '12345', 'text': 'Hello, Twitter!'})
    bot = TwitterBot()
...
def test_post_tweet_api_error(mock_create_tweet):
    mock_create_tweet.side_effect = tweepy.TweepyException("API Error 401: Unauthorized")
    bot = TwitterBot()
